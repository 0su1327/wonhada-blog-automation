...
> 사진과 짧은 메모만으로 AI가 블로그 초안을 쓰고, 브라우저 자동화가 네이버 블로그에 직접 임시저장까지 해주는 개인 프로젝트입니다. 어머니가 운영하는 필라테스·요가 스튜디오의 블로그 운영 부담을 줄이기 위해 기획했고, 실제로 본인 블로그를 대상으로 처음부터 끝까지 동작을 검증했습니다.

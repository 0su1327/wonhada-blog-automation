...
**사진 몇 장 + 메모 한 줄 → AI가 글을 생성하고 자동으로 네이버 블로그에 게시글이 저장.**

어머니가 운영하는 필라테스 스튜디오의 블로그 운영 부담을 줄이기 위해 기획하고, 처음부터 끝까지 직접 구현·검증한 프로젝트입니다.
...
| **해결** | 사진+메모만 입력하면 AI가 글을 쓰고, 브라우저 자동화가 블로그 글쓰기에서 직접 타이핑해 저장 |
| **핵심 기술** | Claude API · n8n · Playwright · Docker · Vercel |
| **결과** | 전체 소요 시간 **30~40분 → 5분 내외 (체감상 80%↓)**, 실제 블로그 대상 end-to-end 검증 완료 |
...
- **환경 설정 단계**: PowerShell 실행 정책 오류, Playwright 라이브러리 버전과 Docker 베이스 이미지 버전 불일치
- **인증/세션 관리**: 컨테이너 내부에는 화면이 없어 로그인 브라우저를 띄울 수 없다는 점을 뒤늦게 파악 → 로그인 과정을 호스트(Windows)에서 수행하고 세션 파일만 컨테이너로 복사하는 구조로 재설계. 
- **웹 자동화 특유의 문제**: 네이버 스마트에디터를 실제 DOM 검사로 명확하게 구조화, 안내 팝업이 버튼 클릭을 가로막는 문제, `Tab` 키로 필드 이동이 불안정해 제목과 본문 텍스트가 섞이는 문제
- **LLM 응답 처리**: `max_tokens` 제한으로 응답이 중간에 잘려 JSON 파싱이 깨지는 문제 → 토큰 한도 상향 + 파싱 실패 시 깨진 내용을 그대로 게시하지 않고 명확히 에러 처리하도록 안전장치 추가

디버깅 과정에서 얻은 원칙: **n8n 파이프라인 실패 시점의 화면을 스크린샷으로 자동 저장**하도록 만들어두니, "왜 안 되는지"를 추측하는 대신 실제 화면을 보고 원인을 특정할 수 있어 디버깅 속도가 크게 빨라졌습니다.
...
**엔드투엔드로 요구사항 파악 → 설계 → 구현 → 배포 → 운영 중 발생하는 문제 해결**까지 전체 소프트웨어 개발 사이클을 혼자 진행해본 경험이라는 점에서, 실무 소프트웨어 엔지니어링 역량을 보여줄 수 있는 프로젝트라고 생각합니다.
